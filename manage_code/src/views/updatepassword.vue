		...
		passwordFormRef.value.validate(async (valid) => {
			if (valid) {
				if(sessionTable.value == 'users'){
					...
					if(sessionTable.value == 'cangguanyuan'){
						var nowpassword = ''
						await context?.$http({
							url:'encrypt/sha-2?text=' + form.value.mima1,
							method: 'get'
						}).then(res=>{
						...
						if(form.value.mima.length<6){
							context?.$toolUtil.message('新密码长度不能小于6','error')
							return false
						}
						if(form.value.mima.length>10){
							context?.$toolUtil.message('新密码长度不能大于10','error')
							return false
						}
					}
					if(sessionTable.value == 'xiaoshourenyuan'){
						var nowpassword = ''
						await context?.$http({
							url:'encrypt/sha-2?text=' + form.value.mima1,
							method: 'get'
					...
					if(sessionTable.value == 'caigourenyuan'){
						var nowpassword = ''
						await context?.$http({
							url:'encrypt/sha-2?text=' + form.value.mima1,
							method: 'get'
