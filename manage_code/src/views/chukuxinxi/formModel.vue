		...
		<el-dialog modal-class="edit_form_modal" custom-class="edit_form" v-model="formVisible" :title="formTitle" width="80%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					...
					<el-col :span="12">
						<el-form-item label="出库日期" prop="chukuriqi">
							<el-date-picker
								class="list_date"
								...
								type="datetime"
								:readonly="!isAdd||disabledForm.chukuriqi?true:false"
								placeholder="请选择出库日期" />
						</el-form-item>
					</el-col>
					<el-col :span="12">
...
<script setup>
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		resetForm()
			form.value.chukuriqi = context?.$toolUtil.getCurDateTime()
		if(formId){
			id.value = formId
			type.value = formType
		}
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='chukuriqi'){
					form.value.chukuriqi = row[x];
					disabledForm.value.chukuriqi = true;
					continue;
				}
	...
	const save=()=>{
		...
		if(objcross!='') {
			...
			if(objcross.shuliang<0){
				context?.$toolUtil.message('数量不足','error')
				return false
			}
		}
		let crossUserId = ''
