	...
	<div>
		<div class="register_view">
			<el-form :model="registerForm" class="register_form">
				...
				<div class="list_item">
					<div class="list_label">销售账号：</div>
					<el-input class="list_inp"
						 v-model="registerForm.xiaoshouzhanghao"
						 placeholder="请输入销售账号"
						 type="text"
						/>
				</div>
				...
				<div class="list_item">
					<div class="list_label">销售姓名：</div>
					<el-input class="list_inp"
						 v-model="registerForm.xiaoshouxingming"
						 placeholder="请输入销售姓名"
						 type="text"
						/>
				</div>
				...
				<div class="list_item">
					...
					<el-select 
						...
						placeholder="请选择性别"
						>
						<el-option v-for="item in xiaoshourenyuanxingbieLists" :label="item" :value="item"></el-option>
					</el-select>
				</div>
...
<script setup>
	...
	const tableName = ref('xiaoshourenyuan')
	...
	const xiaoshourenyuanxingbieLists = ref([])
	const init=()=>{
		xiaoshourenyuanxingbieLists.value = "男,女".split(',')
	}
	...
	const handleRegister = () => {
		...
		if((!registerForm.value.xiaoshouzhanghao)){
			context?.$toolUtil.message(`销售账号不能为空`,'error')
			return false
		}
		...
		if(registerForm.value.mima&& (String(registerForm.value.mima).length<6)){
			context?.$toolUtil.message(`密码长度不能小于6`,'error')
			return false
		}
		if(registerForm.value.mima&& (String(registerForm.value.mima).length>10)){
			context?.$toolUtil.message(`密码长度不能大于10`,'error')
			return false
		}
		...
		if((!registerForm.value.xiaoshouxingming)){
			context?.$toolUtil.message(`销售姓名不能为空`,'error')
			return false
		}
